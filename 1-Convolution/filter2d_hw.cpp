        ...
        hls::stream<U8>     &pixel_stream )
{
    ...
    // Apply array partitioning to the coeffs array to improve parallelism
    #pragma HLS ARRAY_PARTITION variable=coeffs complete dim=2

    // Load the coefficients into local storage
    load_coefs: for (int i=0; i<FILTER_V_SIZE; i++) {
        Filter2D_label0:for (int j=0; j<FILTER_H_SIZE; j++) {
...
#pragma HLS ARRAY_PARTITION dim=2 type=complete variable=w.pix
           // Unroll inner loops to improve performance
